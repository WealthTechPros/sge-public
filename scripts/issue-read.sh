...
_ir_github_dependencies() { # <number>
  ...
  body="$(gh issue view "$1" --json body --jq '.body' 2>/dev/null)" || return 0
  dep_nums="$(printf '%s' "$body" \
    | grep -ioE '(depends[ -]?on|blocked[ -]?by|requires)[[:space:]:]+#[0-9]+' \
    | grep -oE '[0-9]+' | sort -un)" || return 0
...
_ir_forgejo_dependencies() { # <number>
  ...
  body="$(_ir_forgejo_view "$origin" "$1" | jq -r '.body // ""')" || return 0
  dep_nums="$(printf '%s' "$body" \
    | grep -ioE '(depends[ -]?on|blocked[ -]?by|requires)[[:space:]:]+#[0-9]+' \
    | grep -oE '[0-9]+' | sort -un)" || return 0
  for n in $dep_nums; do
    state="$(_ir_forgejo_view "$origin" "$n" | jq -r '.state // "OPEN"')" || continue
    case "$state" in
      CLOSED|closed) printf '%s\tclosed\n' "$n" ;;
      *)             printf '%s\topen\n' "$n" ;;
    esac
...
# the same character class the Jira path enforces ([A-Za-z0-9._-]+); an
# invalid value is treated as "no label filter" (empty).
_ir_github_dispatch_label() {
  local lbl
  lbl="$(grep -E '^dispatch-label:\s*\S' CLAUDE.md 2>/dev/null \
