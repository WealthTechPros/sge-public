...
# ---------------------------------------------------------------------------------------
# Single source of truth for `/sge:sge-align`'s docs-coverage fleet metric, folded
...
#     a `ref:` matching SPEC-/SGE-. It is "current-documented" iff it has a non-empty
...
# nested SGE model includes domain-level nodes — an over-count that is honest and
...
# Emits nothing unless the file has front-matter with a ref: SPEC-/SGE-.
parse_spec() {
  awk '
    ...
    END{
      ...
      gsub(/^[ \t]+|[ \t]+$/,"",cap)
      gsub(/^[ \t]+|[ \t]+$/,"",st)
      if (ref ~ /^(SPEC|SGD|SGE)-/) printf "%s\t%s\t%s\t%d\n", ref, cap, tolower(st), body
    }
  ' "$1"
}
