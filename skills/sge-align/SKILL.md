...
| **C21** (unified **C34**) | UX/e2e coherence (SPEC-078) | a `user_facing` Feature marked `built`/`building` with no Playwright scenario referencing its Feature/Spec ID — run through the SPEC-089 applicability seam (N/A when no Feature is `user_facing: true`) — **advisory, warn-only** — mechanism: `references/check-mechanisms.md` |
| **C35** | Mutation-testing coherence (SPEC-085) | a spec with mapped `sourcePaths` whose mutation-test kill rate is below its own per-spec threshold — **advisory, warn-only, scheduled/on-demand evidence, not derived per-PR** — mechanism: `references/check-mechanisms.md` |
