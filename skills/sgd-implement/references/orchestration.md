# sgd-implement — orchestrator dispatch & front-loaded-verdict reuse (reference)
...
`/sgd:team-pipeline`, `/sgd:issue-swarm`, or a one-off `Agent()` call handing
off "implement issue N end-to-end"), its own Phase 7 already drives the
resulting PR through `/sgd:pr-review` — fixing findings, re-reviewing, and
arming auto-merge on a clean pass — as part of *this* skill's execution. The
dispatching orchestrator must **not** also independently invoke `/sgd:pr-review`
...
When this skill runs as a dispatched subagent (Tier-0 fan-out,
`/sgd:team-pipeline`, `/sgd:issue-swarm`, or a one-off `Agent()` "implement issue
...
*also* fire a separate, parallel `/sgd:governance-trace` on the same issue "to
...
`/sgd:build-ready-audit`, whose #872 Step-2G fold already produces exactly this
...
was already computed upstream and handed to you. `/sgd:build-ready-audit` (issue
#872) folds `/sgd:governance-trace`'s classification into its Step 2G, so an
...
- **How it is passed.** The orchestrator sets `SGD_GOVTRACE_VERDICT` to either an
  inline JSON string or a path to a JSON file carrying `/sgd:governance-trace`'s
...
- **Adopt only on an exact issue match.** Parse it; if its `issue` equals this
  ...
  starting map so the saving is auditable. If `SGD_GOVTRACE_VERDICT` is unset,
...
- **Reuse is not a bypass.** A reused verdict enters the **exact same**
  ...
  reused `MATCHES_EXISTING_MODIFIED`, `NEEDS_NEW_SPEC`, `NOT_SGD_SCOPE`, or
  ...
  for a reused `MATCHES_EXISTING_MODIFIED`/`NOT_SGD_SCOPE` verdict the
...
- **Dispatch, don't await.** Fire the `/sgd:governance-trace` fork (or, on the
...
- **Blocked issues never reach implementation.** A `MATCHES_EXISTING_MODIFIED`,
  `NEEDS_NEW_SPEC`, `NOT_SGD_SCOPE`, or `matchConfidence: "low"` verdict
...
FORK_OUTPUT="/tmp/sgd-govtrace-${FORK_HANDLE}.json"
# Persist the id so the Phase 3 join (a different shell) reads it back:
printf '%s\n' "$FORK_HANDLE" > "/tmp/sgd-govtrace-handle-${ISSUE}"
...
```bash
ISSUE=<issue-number>
FORK_HANDLE=$(cat "/tmp/sgd-govtrace-handle-${ISSUE}")
...
`NOT_SGD_SCOPE`, `matchConfidence: "low"`, timeout, or unknown handle → halt before
...
When dispatched by `/sgd:team-pipeline` or `/sgd:issue-swarm`, a governance pause
...
`/sgd:governance-trace`'s Step-7 verdict object, as returned to Phase 0.5:
