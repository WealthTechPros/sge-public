# sgd-implement — Phase 2.5 complexity-tiered scoped read (extended rationale)
...
1. **Digest (always, every tier).** Read `docs/sgd-digest.md` (location per repo
   CLAUDE.md; format contract: `docs/sgd-digest-schema.md` in the sgd repo) —
...
fresh-context subagent** (`/sgd:sgd-review`) to verify the change. That fork is
...
3. **Side-effect check** — compare `git diff --name-only origin/main...HEAD`
   ...
   - Immediately **escalate to standard-tier**: spawn a forked `/sgd:sgd-review`
     subagent exactly as for a `standard`-tier change.
   - Record `verification_mode = "subagent (escalated from trivial)"`.
   - The PR description (Phase 6 `sgd-phase5-verdict` comment) carries this value
...
4. **Auditable outcome** — once verification completes (inline or escalated), set
   ...
   - `"subagent"` — a forked `/sgd:sgd-review` ran (standard/critical, or
   ...
   `sgd-phase5-verdict` HTML comment, making it visible to `/sgd:pr-review` and
