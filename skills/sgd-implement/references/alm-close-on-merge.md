...
| **jira** | Records the merge as a development-panel **remote link** on the item (`globalId` = the change URL, so a re-run upserts rather than duplicating), plus the close transition when `SGD_JIRA_CLOSE_TRANSITION_ID` is configured. A non-2xx write **fails loud** — never silently swallowed. |
