...
unattended, it reads the three SGD drift **trend surfaces**, picks the single
highest-leverage gap across them, runs the matching `/sgd:drift-hillclimb`
dimension for **one bounded PR**, re-measures, and appends the measured delta.

- Skill: [`SKILL.md`](./SKILL.md) — `/sgd:improvement-sweep`
...
|---|---|---|
| coherence | `docs/sgd/drift-trend.jsonl` (Audit Score `audit_score`; legacy `sm2_sample`) | `/sgd:drift-hillclimb --max-rounds 1` |
| token-economy | `docs/sgd/token-economy-trend.jsonl` (`score-token-economy.mjs` `trendRow`, #831) | `/sgd:drift-hillclimb --dimension token-economy --max-rounds 1` |
| skill-quality | `docs/sgd/skill-quality-trend.jsonl` (`score-skill-quality.mjs` `trendRow`, #832/#737) | `/sgd:drift-hillclimb --dimension skill-quality --max-rounds 1` |
...
- **Every cycle appends a measured delta** — one row per cycle is appended to
  `docs/sgd/improvement-sweep.jsonl` via `buildCycleRecord()`, carrying
...
2. **`act`** — runs only when a dial was selected, it is not a dry-run, **and**
   ...
   `/sgd:improvement-sweep`, which performs the bounded climb, re-measure, and
...
a control session can run `/sgd:improvement-sweep` to act on it. This keeps the
...
/sgd:improvement-sweep            # act on the highest-leverage dial
/sgd:improvement-sweep --dry-run  # select + report only; open nothing
```
