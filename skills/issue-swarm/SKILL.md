---
description: Use when you want an autonomous, time-boxed run that continuously works build-ready SGE issues to reviewed, green PRs and stops cleanly when a wall-clock budget expires or the queue drains. Invoke when the user says "swarm the issues for the next 2 hours", "burn down the backlog until lunch", "run an unattended SGE session with a hard stop", or wants duration-bounded parallel issue progress that never overruns.
argument-hint: "[--duration <Nm|Nh>] [--agents N] [--module <name>] [--milestone <name>] [--dry-run]"
---

# /issue-swarm — Router to /sge:team-pipeline --duration

## Role
Route a duration-bounded swarm request to `/sge:team-pipeline --duration`, which owns the time-boxed engine end-to-end.
...
`/sge:sge-implement` to a reviewed, green PR against its own lean-contract
...
`/sge:team-pipeline`. When passing through to it, this skill **does not
re-detect the host** — `/sge:team-pipeline` detects the host at its own
...
beyond `[A-Za-z0-9._-]`) before forwarding it to `/sge:team-pipeline`. A
