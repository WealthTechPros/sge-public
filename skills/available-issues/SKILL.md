...
`IR` (`scripts/issue-read.sh`, #1237) is the host-aware seam for all issue read operations in this skill: it routes to `gh` for GitHub and to `scripts/forgejo-adapter.sh` for Forgejo/Gitea, with a normalised JSON output shape so the rest of the skill is host-agnostic.
...
if [ -n "$DISPATCH_LABEL" ]; then
  ...
  CANDIDATES=$("$IR" list --state open --limit 100 --label "$DISPATCH_LABEL" \
    | jq '[.[] | select(
      ...
      ([.labels[].name] | index("orchestrator-only") | not)
    )] | sort_by(.number)')
