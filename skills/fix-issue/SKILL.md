---
description: Use when fixing a GitHub issue that reports a production bug or regression — routes to /sgd:sgd-implement, with Sentry context (stacktrace, breadcrumbs, environment) gathered as pre-context when a Sentry issue ID is referenced.
...
## Role
Route a production bug or regression issue to `/sgd:sgd-implement`, which owns the full end-to-end pipeline — enriched with live Sentry telemetry as diagnosis pre-context when available.
...
- CI/PR-level fixes — red builds on an existing PR (use `/sgd:pr-fix`)
- Incidents requiring on-call runbooks (see `/sgd:prod-reliability-playbook`)

This skill no longer owns an implementation pipeline. `/sgd:sgd-implement` handles bug-fix issues end-to-end — worktree isolation, TDD via `/sgd:tdd-workflow` (failing test first), independent review, commits via `/sgd:commit`, and the PR-review + fix loop that the old pipeline here lacked.
...
3. Dispatch to the no-spec fix lane (`fix/` branch taxonomy, `SGD-Override` trailer), attaching any Sentry pre-context to the diagnosis. With or without Sentry context, invoke:
