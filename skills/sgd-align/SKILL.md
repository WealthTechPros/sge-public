...
## Role
Sweep a repo (or fleet) for SGD governance drift, score Zero-Trust and regulatory-traceability dimensions, and file GitHub issues for every broken link — advisory-only, never blocking.
...
| Cortex maintenance (Step 6) | `consolidate` / `reflect` / `audit_read` (sgd-memory, if available) |

Walks the SGD layer cascade **left → right** and finds where the chain breaks:
...
Every broken link is a **drift gap** — intent that never became a capability, a built feature with no spec, a spec with no Gherkin, a scenario with no test, code with no spec, a stale cross-repo contract. The command turns each gap into a tracked **GitHub issue**, so drift becomes work instead of silent rot. This is the local, repo-side companion to the SGD platform checks (SGD-027, SGD-031, SGD-032). **Advisory-first:** it reports and tracks; it never blocks a PR or weakens a check.

## Usage

```
/sgd:sgd-align [--dry-run] [--apply] [--label sgd-drift] [--max 20] [--check C1..C14,C19]
/sgd:sgd-align --dimension agent-security|regulatory|skill-quality   # one dimension only
/sgd:sgd-align --fleet <org>/<repo> [<org>/<repo>…] | --fleet <org>/*
```
...
- `--label` (default `sgd-drift`), `--max` (default 20, cap issues raised per run — log what was deferred), `--check` (run one cascade check).
...
- `--dimension skill-quality` — delegates entirely to `/sgd:sgd-skill-audit --all` (no duplicate logic here); skip C1–C14/C19; emit the Step 5 `skillQuality` JSON only. Not a Cn check — contributes no `gaps[]`, never enters the composite.
...
`docs/sgd/drift-trend.jsonl` (issue #1041). One Bash call, from inside the
resolved checkout:
...
"$IR" list --label sgd-drift --state open --limit 1000 \
...
L0 Vision, L1 Capability Model, L2 Design System, L3 Feature Specs, L4 ADRs, L5 DAG Manifest, L6 Change Protocol, L7 SGD Alerts, L8 Cortex.
- **Swept:** L0 (C1, C2), L1 (C2, C3), L2 via C10 (delegated to `/sgd:atomic-audit`), L3 + acceptance criteria (C3–C6), L4 in part (C7), plus the traceability spine: tests (C5), code presence and content (C6, C13), stakeholder questions (C8), cross-repo contracts (C9).
- **Not swept:** L5 (platform DAG tooling), L6 (hooks/`/sgd:sgd-implement`), L7/L8 runtime — not this sweep's job to surface. **Exception:** L8 gets a lightweight maintenance-only phase (Step 6, issue #678).

## Step 0 — Learn this repo's conventions

Layer artefacts live in different places per repo. **Read the repo's `CLAUDE.md` and `docs/sgd/`** to locate each before checking:
...
| L2 Design System | probed via `/sgd:atomic-audit` (C10), not a file lookup |
...
| Fleet manifest (C9 / fleet mode) | `docs/sgd/fleet.yaml` — `repos: [{name: <org>/<repo>, contracts: [<paths>]}]` |
...
{ "check": "C3", "layer": "L1→L3", "key": "C3:CAP-CLIENT-ONBOARDING-ACCEPT",
  ...
  "proposedIssue": { "title": "[SGD drift] C3 Capability→Spec: …", "body": "…" } }
```
...
| # | Boundary | A gap is… |
|---|---|---|
| **C1** | Vision exists (L0) | no `docs/vision.md`, or missing a required section (Layer-0 seed list, `/sgd:sgd-init` Step 2) |
...
| **C10** | Design System (L2) | `/sgd:atomic-audit` reports maturity tier L0/L1 — mechanism: `references/check-mechanisms.md` |
...
> **Auxiliary (advisory, unscored): plugin-skill shadowing (issue #1066).** A repo-local `.claude/commands/<name>.md` whose basename matches a bundled SGD skill silently shadows `/sgd:<name>` (a bare `/<name>` resolves to the stale local copy). Run `scripts/detect-shadowed-commands.sh <target-repo>` (from a plugin checkout, or `--skills-dir <plugin>/skills`) to list collisions; reconcile each by deleting the stale command or converting it to a thin `/sgd:<name>` wrapper. This is a hygiene warning, not a scored cascade gap.

## Step 2 — Reconcile with existing issues (idempotent — do this BEFORE creating anything)

Every issue this command files carries a hidden stable key: `<!-- sgd-drift-key: C3:CAP-CLIENT-ONBOARDING-ACCEPT -->`.
...
Render each gap's `proposedIssue` via `gh issue create --title … --label "$LABEL" --body …` with the `<!-- sgd-drift-key: … -->` footer — worked example: `references/issue-lifecycle-examples.md`. Respect `--max`: file highest-severity first, log the deferred count — never silently truncate.
...
**Trend persistence.** By default, every full sweep appends its Step 5 JSON as one line to `docs/sgd/drift-trend.jsonl` (create `docs/sgd/` if missing) and prints the Audit Score delta against the previous row — the single durable Audit Score record `/sgd:drift-hillclimb` diffs. Commit this file (via `/sgd:commit`) each sweep. **Skipped** in standalone `--dimension` modes (their JSON has no `audit_score`/`checks[]`). Mechanism and commands: `references/scorecard-and-trend.md`.

## Step 6 — Cortex maintenance (Layer 8 distillation)

At the end of every full sweep (default and `--apply`), and inside each fleet per-repo agent, run the in-context distillation pass plus the `consolidate` (every run) and `reflect` (weekly, self-timed) sgd-memory tools — best-effort, never blocking, never a `gaps[]` entry, degrades gracefully when sgd-memory is unavailable. **Skipped** in standalone `--dimension` modes. Full mechanism: `references/cortex-maintenance.md`.

## Fleet mode — org-wide sweep

```
/sgd:sgd-align --fleet wtp/sgd wtp/client-x   # explicit repos
/sgd:sgd-align --fleet wtp/*                  # org glob
/sgd:sgd-align --fleet                        # repos from docs/sgd/fleet.yaml
```

Orchestrate as a **Workflow**: one **read-only audit agent per repo** (cap concurrency ~8), each of which gets the repo locally (existing checkout, else `gh repo clone --depth 1`), runs Steps 0–2 in **dry-run** (fleet agents never file/close issues) then Step 6, and returns a schema-validated Step 5 JSON (malformed → one retry, then `error` in the roll-up). The orchestrator aggregates a **fleet Audit Score scorecard** (per-repo rows worst-first, org Audit Score = mean of per-repo `audit_score`, per-check fleet pass rates) — this aggregate is the fleet **Audit Score** rollup, an operational audit signal. It is **not** the canonical SM-2: the SGD Vision tracks SM-2 as the platform's `coherence_score` composite (SGD-032-S8 / #883), which this plugin does not compute. Issue mutations happen only under `--apply`, sequentially, post-audit.

**Recurring cadence.** The Audit Score is a trend, not a snapshot — wrap `--fleet` in `/loop <interval> /sgd:sgd-align --fleet …` (weekly) and commit `docs/sgd/drift-trend.jsonl` each run so the next sweep has something to diff against.

## Safety

- **Advisory-first.** Files/updates issues; never blocks a PR or weakens a check.
- **Human issues are sacred.** Step 4 only proposes by default; mutating a human-filed issue needs `--apply` (or in-session confirmation), always with a cited-rationale comment first. Only the command's own `sgd-drift` issues are auto-managed.
- **Idempotent.** Dedupe by `sgd-drift-key`; close gaps that are fixed; re-running is safe — why `context: fork` is set.
...
See also `/sgd:sgd-preflight` (per-spec readiness), `/sgd:sgd-review` (per-PR coherence), `/sgd:atomic-audit` (the L2 probe C10 consumes), `/sgd:sgd-init` (seeds the artefacts this sweep checks), and `/sgd:drift-hillclimb` (closes gaps this sweep finds).
