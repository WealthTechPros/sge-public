---
description: Use when installing the SGD Traceability Module (SPEC-054) into the current repo — copies the traceability data builder and interactive chart page, wires the docs Pages workflow, and opens a draft install PR. Run from inside a cloned SGD-governed repo checkout when the user asks to "install the traceability chart/module". For the web-UI install use the SGD Modules tab instead; this skill does not build or audit traceability data itself.
...
You are the SGD Traceability Module installer. Your job is to copy the module's assets (`build-traceability.mjs`, `traceability.html`) into the current repo, insert the `Build traceability data` step into the repo's Pages workflow, and open a draft install PR for the operator to review and merge.
...
Install the SGD Traceability Module into the current repo.
...
4. Commits on a new branch `sgd/install-traceability`
5. Opens a draft PR: `feat(sgd): install SGD traceability chart (SPEC-054)`

## Prerequisites

- You are inside a cloned SGD-governed repo checkout.
- The SGD plugin is installed (`/plugin install sgd` if not).
...
# 2. Copy module assets from the SGD plugin
cp "$(sgd plugin-root)/skills/traceability/build-traceability.mjs" scripts/
cp "$(sgd plugin-root)/skills/traceability/traceability.html" docs/
...
# 5. Commit and push
git checkout -b sgd/install-traceability
git add scripts/build-traceability.mjs docs/traceability.html docs/assets/traceability.json
git commit -m "feat(sgd): install SGD traceability chart (SPEC-054)"
git push -u origin sgd/install-traceability
gh pr create --title "feat(sgd): install SGD traceability chart (SPEC-054)" --draft \
  --body "Installs the SGD Traceability Module per SPEC-054. Adds the data builder script and the interactive chart page. Add the CI step from INSTALL.md to your docs workflow to keep traceability.json fresh."
```
...
- `SPEC-054-sgd-traceability-module.md` — full spec
