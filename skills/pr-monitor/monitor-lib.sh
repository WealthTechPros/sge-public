...
# extraction of issue #819 — the function bodies are verbatim from the skill.
#
...
#                                label — the pool the stale-claim takeover
#                                re-examines
#   claim_labeled_epoch <pr>     epoch seconds of the MOST RECENT claim-label
#                                `labeled` timeline event — when the current
...
#                                terminal-red state set) — so a stall can be
...
#                                required check — a live review lane stuck on a
...
#                                (<30s with no real work) — rerun, don't fix
#   is_setup_step_html_error <run_id>
#                                0 iff any setup step in the run emitted
#                                `<!DOCTYPE html>` in its log — second outage
...
#   SPEC_GLOB_RE          REQUIRED by is_spec_pr / check_systemic_failure —
...
# STALE_DRAFT_MINUTES: how old a draft's LAST COMMIT must be — with no CI in
# flight — before the stale-draft lane (issue #1248) presumes the implementer
...
# newer commit — or any queued/running check — is presumed still alive.
...
# Companion query: open PRs that ARE currently claimed — the pool the stale-claim
...
# (or by being a normal non-draft PR mid-review), so any claimed PR — draft or
# not — is fair game for reclaim_if_stale's own freshness check.
fetch_claimed_prs() {
...
# Epoch seconds of the MOST RECENT claim-label `labeled` event on a PR — i.e.
...
# Prints nothing when no such event exists or the timestamp is unreadable — the
# caller MUST treat empty output as "unknown → FRESH" (never reclaim on missing
...
claim_labeled_epoch() {
  ...
  # GNU date (-d) || BSD/macOS date (-j) — gh emits ISO-8601 trailing-Z UTC.
...
# same head (a fresh `labeled` event → a fresh clock), and it survives pod
# restarts (the timeline is server-side, not pod-local state). Unknown claim age
# → FRESH → strict mutex (return 1); never reclaim on missing data.
#
...
is_stale_claim() {
  ...
  # GNU date (-d) || BSD/macOS date (-v) — bound the freshness window.
  cutoff=$(date -u -d "-${STALE_CLAIM_MINUTES} min" +%s 2>/dev/null \
        || date -u -v-"${STALE_CLAIM_MINUTES}"M +%s)
  claimed_epoch=$(claim_labeled_epoch "$pr")
  # No readable claim-labeled event → treat as FRESH (never reclaim on missing
...
reclaim_if_stale() {
  ...
  if is_stale_claim "$pr"; then
    # LOG IT LOUDLY — a dead session's claim is being taken over.
    echo "WARNING: PR #$pr — stale pr-reviewing claim (>${STALE_CLAIM_MINUTES}m, no new commit AND no activity); reclaiming for this session" >&2
    ...
    # clears any stale pr-reviewed — the canonical, mutex-safe way to take over.
    ${CLAUDE_PLUGIN_ROOT}/skills/pr-review/pr-labels.sh start-review "$pr"
    return 0   # eligible — enter the lane and review it
  fi
  return 1     # FRESH claim → strict mutex: skip and re-check next cycle
}
...
# terminal-failure states gh can report — CANCELLED, ACTION_REQUIRED,
# STARTUP_FAILURE, STALE — so a held review stuck on any of them is still seen.
...
# Print the NAMES of failing required checks, one per line. Empty output → no
...
# (0x1F / \037) — NOT a tab: tab is bash IFS-whitespace, so an empty field (e.g.
...
#   KNOWN:<policy>   a valid (unexpired), in-scope entry matches — reporting-only
...
classify_failing_check() { # $1=check name  [$2=today YYYY-MM-DD]  [$3=registry file]
  ...
  # \x1f (Unit Separator) delimiter — see known_flakes_rows(): a non-whitespace
...
held_review_stall() {
  ...
  # Must currently carry a claim label — else there is no held review to stall.
  claimed=$(gh pr view "$pr" --json labels \
    --jq --arg claim "$CLAIM_LABELS_RE" '[.labels[].name] | any(test($claim))' 2>/dev/null)
  [ "$claimed" = "true" ] || return 1
  # A DEAD claim is the stale-claim takeover's job, not this one — only a FRESH
  ...
  # throw here or be read as a stall) — issue #1206.
...
#     someone else's lane — the #755 orphan carve-out / normal review flow), AND
...
#     WAITING) — a running check means the implementer's CI may still finish and
...
# Act on an abandoned draft (issue #1248) — NEVER silently.
...
# active draft" is an invariant of THIS function, not merely the caller's — a
...
stale_draft_lane() {
  ...
  if [ "$failing" -eq 0 ]; then
    # GREEN — mark ready, log loudly, and leave an audit trail.
    echo "WARNING: PR #$pr — abandoned GREEN draft (no commit for >${STALE_DRAFT_MINUTES}m, CI green, no in-flight checks); marking ready for review" >&2
    gh pr ready "$pr" >/dev/null 2>&1
    body=$(printf '%s\n**Abandoned draft — auto-marked ready (issue #1248).** This draft had no new commit for over %s minutes and its CI is green with no checks in flight, so its implementer is presumed to have died before running `gh pr ready`. Marking it ready for review so a review lane will pick it up. If this was premature, convert it back to a draft.' "$marker" "$STALE_DRAFT_MINUTES")
    gh pr comment "$pr" --body "$body" >/dev/null 2>&1
    return 0
  fi
  # RED / incomplete — surface it (idempotent per head), never mark ready.
  existing=$(gh api --paginate "repos/{owner}/{repo}/issues/$pr/comments" \
    --jq "[.[] | select(.body | contains(\"$marker\"))] | length" 2>/dev/null \
    | awk '{s+=$1} END{print s+0}')
  if [ "${existing:-0}" -gt 0 ]; then
...
pr_ready_for_merge() {
  ...
  # Gate 2 — merge-gate label present
  ...
  # Gate 3 — CI green. Use the SHARED terminal-red set ($FAILING_CHECK_JQ, defined
  ...
  # narrow pair let it PASS Gate 3 — the PR would report READY and arm after the
...
# Returns 0 (true) when a run was CANCELLED or TIMED_OUT — a structural
...
# setup/checkout step was `cancelled` — the suite never executed, so there is no
...
# by `gh run rerun --failed` — re-running its failed jobs leaves the RUN's
# conclusion `cancelled` and the check red (observed: three `--failed` reruns,
# one cancelled run at head, zero progress). The escape is a FRESH run —
# `gh pr update-branch` (also fixes the usually-stale base) or a full
# `gh run rerun` (no `--failed`) — see the CANCELLED row in SKILL.md.
#
...
is_cancelled_run() {
  ...
  # test-shaped step is `skipped` in the same job — the suite never ran, so this
...
# `gh run rerun --failed` is the right cheap action — a runner casualty, NOT a
# cancellation. A CANCELLED/TIMED_OUT run is deliberately EXCLUDED here (it is
# its own state — `is_cancelled_run`, CANCELLED row — because `--failed` can
...
# The duration test is kept only as a WEAK secondary hint per issue #1665 —
...
is_infra_failure() {
  ...
  # A cancelled/timed-out run is NOT rerun-with-failed infra — route it to the
  ...
  [[ "${max_duration:-999}" -lt 30 ]]   # <30s with no real work → INFRA FAIL
}

# Escape a cancelled/timed-out run — force a FRESH run rather than reruning failed
...
# local worktree holds (`update_branch_safe` guards it) — a rebase would strand
# that worktree; re-sync it (see `worktree_sync_state`) before further work.
#
# Rerun cap (issue #1665): a cancelled run must not be retried indefinitely — the
...
# and echoes `noop:rerun-cap-reached` — the caller must escalate (a genuinely new
...
# that state force-pushes a tree missing everything the rebase pulled in — the
...
# healthy — clean status, right branch — the only tell is a HEAD that disagrees
...
# whatever repo the process cwd happens to be — the sync helpers pass the same
# dir they operate on.
...
# <branch> AND its HEAD equals origin's tip for that branch — i.e. it is safe to
...
worktree_synced_with_remote() { # $1=branch  [$2=worktree dir]
  ...
  # race is benign — this helper only READS (no fetch/reset follows it), and a
...
# — the PREVENTION rule for issue #1666 (chosen over an autonomous in-library
...
# on such a branch from the monitor — resolve the conflict without it, or have the
# human re-sync the worktree first.**
#
# Returns 0 (SAFE to update-branch) iff NO local worktree is currently checked out
# on <branch>; returns 1 (UNSAFE — a worktree holds it) otherwise, echoing the
...
post_update_branch_blocked_comment() { # $1=pr  [$2=holder worktree path]
  ...
  body=$(printf '%s\n**CONFLICTING — update-branch withheld (issue #1666).** This PR is behind its base, but the monitor is **not** running `gh pr update-branch` because a local worktree is checked out on its branch%s. Rebasing the remote while a worktree holds the branch strands that worktree on the pre-rebase commit — an agent could then push a tree that reverts merged work (the #1666 near-miss). **Action:** re-sync (or free) that worktree to the branch tip, then the monitor rebases on the next cycle. Surfaced once per head-SHA so this is not a silent stall.' "$marker" "${holder:+ (\`$holder\`)}")
  gh pr comment "$pr" --body "$body" >/dev/null 2>&1
}

# Report whether the worktree at <dir> needs a manual re-sync after an
# update-branch that already happened elsewhere — a READ-ONLY diagnostic (no
# mutation, so no destructive action lives in this library). Echoes exactly one
# of: `synced` (HEAD == origin tip — safe to commit/push), `stale:<remote_sha>`
# (behind origin — the caller/human must re-sync this worktree to <remote_sha>
# before any further work; see SKILL.md), `dirty` (uncommitted changes — never
# auto-touch), or `unknown` (unreadable — treat as unsafe). Fail-closed: anything
...
# label applied 17:37:53, armed 17:38:22 (29s later), label RETRACTED 17:42:05 —
...
# was `unlabeled`) or the timeline is unreadable — the caller MUST treat empty as
...
# least AUTOMERGE_SETTLE_SECONDS (default 300) — the window a self-correcting
...
# the merge-gate label — the disarm worklist (issue #1668, Defect 1). One
...
# armed_without_gate_label, disable auto-merge and log loudly. Idempotent — a PR
...
disarm_stale_automerge() { # [$1=label]
  ...
  while IFS= read -r pr; do
    ...
    if gh pr merge "$pr" --disable-auto >/dev/null 2>&1; then
      echo "WARNING: PR #$pr — auto-merge was ARMED but the '$label' gate label is gone; disarming (issue #1668)" >&2
      echo "disarmed:$pr"
    else
      # Disarm FAILED — the PR is still armed with no gate label. Never swallow
      ...
      echo "WARNING: PR #$pr — FAILED to disable auto-merge (still ARMED without the '$label' gate label); needs operator attention (issue #1668)" >&2
...
# in its log — a second outage signal for the GITHUB DEGRADED classification
...
# (not degraded) to avoid false positives — never classify GITHUB DEGRADED on
# missing data.
...
# unreachable/unresolvable predicate maps to "error" — a degraded state — so a
...
# string, so the controlled marker — no jq metacharacters — is inlined).
#
...
# The canonical carve-out condition list lives in SKILL.md Appendix A —
# keep this function and that table in lockstep.
is_blast_radius_pr() {
  local pr=$1
  # Cache the file list — one API call, not four.
  local files
