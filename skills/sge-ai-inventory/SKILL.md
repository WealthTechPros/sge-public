...
- Show the full proposed diff. On approval, commit via /sge:commit.
...
7. Show the full proposed additions. On approval, commit via /sge:commit.
...
- **No GitHub / no SGE artefacts:** the skill needs only a writable repo; SGE integration (CLAUDE.md path registration, /sge:commit) degrades to plain file edits the user commits themselves.
