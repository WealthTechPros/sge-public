---
description: Use when you want to point an agent at a repo's backlog and drain it one issue at a time — each build-ready issue worked through the full SGD pipeline (implement → review gate → merge) until /sgd:available-issues --mode autonomous-next returns no next issue. Invoke when the user says "work the backlog one issue at a time until it's done", "keep implementing the next issue until none are left", or wants a queue-empty-bounded (not duration-bounded) unattended serial run. For parallel or time-boxed runs use /sgd:issue-swarm or /sgd:team-pipeline instead.
argument-hint: "[--repo owner/repo] [--max-issues N] [--module <name>] [--milestone <name>] [--merge-wait|--no-merge-wait] [--duration Nh] [--dry-run]"
---

# /sgd:issue-loop — Serial Issue-Drain Loop

## Role
Drain a repo's build-ready backlog **one issue at a time** through the full SGD pipeline — pick, gate, dispatch a full `/sgd:sgd-implement`, confirm the independent review gate, land, advance — until the queue is empty or a bound stops the run. This is the serial counterpart to `/sgd:issue-swarm`: queue-empty-bounded where the swarm is duration-bounded, one full-pipeline agent at a time where the swarm fans out lean agents. It is the consumer `/sgd:available-issues --mode autonomous-next` was designed for.
...
## Out of scope
- **Parallelism of any kind** — that is `/sgd:issue-swarm` / `/sgd:team-pipeline`. This loop never has two implementation agents alive at once.
- **Implementing issues inline** — the driver session dispatches and orchestrates; it writes no code itself.
- **Owning the merge gate** — `pr-reviewed` and auto-merge belong exclusively to `/sgd:pr-review` (via the dispatched pipeline). The driver confirms the gate ran; it never applies labels or arms merges itself.
...
| Part | This skill |
|---|---|
| **Trigger** | Manual first (`/sgd:issue-loop`); optionally recurring cross-session via `/loop <interval> /sgd:issue-loop …` ([loops §D](../loops/SKILL.md#d-recurring--cross-session-loop)) once proven on a repo. |
| **Goal** | **Queue-empty** — `/sgd:available-issues --mode autonomous-next` returns `{"issue": null}`. |
| **Work Unit** | One issue → merged, reviewed PR (or a recorded skip). |
| **Verifier** | The `/sgd:pr-review` gate + CI green — independent of the implementing agent, never self-graded. The driver **confirms the gate ran**; it does not re-run it (sgd#699). |
...
| **Governor** | `/sgd:env-health --preflight` before **every** dispatch; per-issue token budgets inherited from `/sgd:sgd-implement`; the thrash/systemic bounds above; never weaken a gate to drain faster ([loops §C](../loops/SKILL.md#c-bounded-refinement-loop)). |

## Composition

| Step | Command | Role |
|------|---------|------|
| Pick | `/sgd:available-issues --mode autonomous-next` | one machine-readable next issue, or `{"issue": null}` to stop |
| Reconcile | `/sgd:reconcile-worklist` | drop the pick if it is already closed / merged before any dispatch |
| Gate | `/sgd:build-ready-audit` | per-issue go/no-go: `READY` \| `NOT_READY` \| `TOO_LARGE` |
| Decompose | `/sgd:decompose-issue` | split a `TOO_LARGE` issue; children re-enter the pool on the next pick |
| Implement | **full `/sgd:sgd-implement <N>`** | the complete pipeline — preflight, TDD, forked review, PR, pr-review loop |
| Verify | `/sgd:pr-review` gate (chained by sgd-implement) | independent merge-gate verdict; the driver confirms, never duplicates |
...
| Shepherd (opt-out path) | `/sgd:pr-monitor` | owns stacked PRs when `--no-merge-wait` is chosen |

The serial loop deliberately pays for the **full `sgd-implement` pipeline** — not the swarm's lean agent contract — because only one agent runs at a time; depth beats fan-out here.

## Usage

```
/sgd:issue-loop                                  # drain the current repo's backlog to empty
/sgd:issue-loop --repo owner/repo                # target another repo (cd/GH_REPO, per SPEC-057)
/sgd:issue-loop --max-issues 3                   # stop after 3 completed issues
/sgd:issue-loop --module auth --milestone "v2"   # scope the queue
/sgd:issue-loop --no-merge-wait                  # stack PRs, hand shepherding to /sgd:pr-monitor
/sgd:issue-loop --duration 4h                    # optional wall-clock ceiling on top of queue-empty
/sgd:issue-loop --dry-run                        # pick + reconcile + gate + plan; dispatch nothing
```
...
| `--module <name>` / `--milestone <name>` | all | Passed through to `/sgd:available-issues` scope filters. |
| `--merge-wait` / `--no-merge-wait` | `--merge-wait` | Default: block until each PR merges, then fast-forward main before the next pick. `--no-merge-wait` stacks PRs and hands them to `/sgd:pr-monitor` — but forfeits serial-group drainage (below). |
| `--duration Nh\|Nm` | off | Optional deadline on top of queue-empty. At the deadline, stop dispatching and let the in-flight issue finish — exactly `/sgd:issue-swarm`'s clean-stop contract (`stopReason: duration`). |
...
- **Preflight gate, every cycle:** run `/sgd:env-health --preflight` before **each** dispatch, not just the first, and honour the verdict: `PASS` → dispatch; `THROTTLE` → dispatch anyway (one agent is already minimum concurrency) but defer any auxiliary background work; `REFUSE` → dispatch **nothing** — wait on the saturation condition with a `Monitor` until-condition ([loops §B](../loops/SKILL.md#b-wait-for-condition-loop); never a foreground sleep), re-gate, and stop per the bounds if the condition never clears.
- **Budget:** per-issue token budgets are inherited from the dispatched `/sgd:sgd-implement` run; the driver itself stays thin (it holds no diffs, no file contents — state lives in GitHub). Consult `/sgd:cost-guard` on long drains.
...
- **Approvals:** every PR goes through the normal merge gate (`/sgd:pr-review` + CI). The driver never applies `pr-reviewed`, never arms auto-merge, never merges by hand.
...
Targeting another repo (`--repo`): any run that **dispatches** (everything except `--dry-run`) must resolve + `cd` into that repo's checkout via the shared helper — `cd "$(${CLAUDE_PLUGIN_ROOT}/scripts/with-repo-cwd.sh resolve owner/repo)" || exit 1` (fail-loud, never falls through to the ambient cwd) — because the dispatched `sgd-implement` worktrees, its quality suites, and step 5's `git pull --ff-only` all resolve against the local checkout, and `GH_REPO` steers `gh` only, never `git` (the raw-`git`-vs-`GH_REPO` split is defined once in [`gh-repo`](../gh-repo/SKILL.md)). `export GH_REPO=owner/repo` alone is valid only for a checkout-less `--dry-run` orchestration pass. Never let `gh` fall through to the ambient cwd repo.

Create the durable skip-label once (idempotent):

```bash
gh label create "loop-skip" --color "E4E669" \
  --description "Skipped by /sgd:issue-loop after repeated failures" 2>/dev/null || true
```
...
```bash
/sgd:available-issues --mode autonomous-next [--module X] [--milestone M]
```
...
/sgd:available-issues [--module X] [--milestone M]   # fallback: full ranked ready list; pick the top non-excluded issue
```
...
```bash
/sgd:reconcile-worklist --issues <N> --repo <owner/repo> --json
/sgd:build-ready-audit <N>
```
...
- `TOO_LARGE` → `/sgd:decompose-issue <N>`, record the parent as decomposed (and add it to the exclusion set if it stays open as a tracking epic), return to step 1 — the children enter the pool and are picked in dependency order on subsequent cycles. Never dispatch an un-split epic.

### 3. Dispatch (the implement step — never inline)

Dispatch **full `/sgd:sgd-implement <N>`** as a **fresh, named, stoppable Task sub-agent** — per the [stoppable-only fan-out rule](../team-pipeline/SKILL.md#stoppable-only-fan-out-rule-mandatory), even though this "fan-out" has width 1:
...
- The sub-agent runs the complete `sgd-implement` pipeline: preflight, TDD, forked pre-PR review, `/sgd:commit`, PR, and its own `/sgd:pr-review` fix loop through to the `pr-reviewed` gate and armed auto-merge.

**Claim before dispatch** — apply the durable cross-agent mutex `available-issues`' claim gate already honours, so a concurrent `/sgd:team-pipeline`, `/sgd:issue-swarm`, or second `issue-loop` never double-picks the issue, and a crash before the sub-agent pushes a branch still leaves a durable trail:
...
Before every dispatch: the Governor's `/sgd:env-health --preflight` gate (above). `REFUSE` → no dispatch.

### 4. Verify independently (confirm, don't double-review)

The dispatched pipeline **chains its own `/sgd:pr-review`** — do not run a second full review from the driver (sgd#699: check for an in-flight/complete review first; a duplicate review burns cost and can race the label mutex). The driver's job is to **confirm the gate actually ran and passed**:
...
If checks settle red, the in-pipeline `/sgd:pr-fix` path has already had its bounded tries — treat the cycle as failed (step 6). If the merge does not land after checks pass (e.g. auto-merge unavailable in the repo), hand the PR to `/sgd:pr-monitor` and count the cycle complete only when it merges; with `--no-merge-wait`, PRs stack and `/sgd:pr-monitor` owns all shepherding (accept that serial groups may then block later picks until merges land).
...
Interrupted mid-run (container reclaim, crash, user stop)? Re-invoking `/sgd:issue-loop` resumes correctly with **no state file**:

- Merged issue A is closed → `available-issues` never re-picks it.
- Mid-flight issue B left a durable trail — a pushed branch, an open PR, an `agent-lock` label — so the claim gate treats it as in-flight/claimed rather than double-claiming; reconcile or finish it via `/sgd:pr-monitor` / `/sgd:pr-fix`, or release the stale claim and let the loop re-pick it.
...
- `/sgd:available-issues` — the pick step (`--mode autonomous-next` is this loop's queue)
- `/sgd:reconcile-worklist`, `/sgd:build-ready-audit`, `/sgd:decompose-issue` — the pre-dispatch filters
- `/sgd:sgd-implement` — the full per-issue pipeline this loop dispatches
- `/sgd:pr-review` — the independent merge gate (confirmed by the driver, owned by the pipeline)
- `/sgd:pr-monitor`, `/sgd:pr-fix` — PR shepherding for `--no-merge-wait` and recovery paths
- `/sgd:env-health` — the preflight Governor gate before every dispatch
- `/sgd:issue-swarm` — the duration-bounded **parallel** sibling; use it for time-boxed fan-out, this loop for a serial drain to empty
