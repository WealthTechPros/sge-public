# No-spec lane (sge-implement 0B)

The 0B lane detail, extracted from `SKILL.md` for the 35 KB size budget (issue #825). Reached from Phase 0.5 only via `NO_SPEC_WARRANTED`, `NOT_ONBOARDED`, or an accepted `NOT_SGE_SCOPE` override.

### 0B: No-spec lane

Reached only via `NO_SPEC_WARRANTED`, `NOT_ONBOARDED`, or an accepted `NOT_SGE_SCOPE` override — never as a default.
...
**Branch taxonomy** (Phase 3, instead of `feat/sge-<NNN>-…`): `feature/issue-<N>-…`, `fix/issue-<N>-…`, `chore/issue-<N>-…`.
