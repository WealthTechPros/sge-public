# sge-implement — splitting an oversized issue into child issues (reference)
...
fan-out is gated on `/sge:build-ready-audit`) stays in `SKILL.md` Phase 2; the
taxonomy, child-creation templates (via the `$IW` write seam), and per-child
gating detail live here.

> **Reached two ways (#1265).** Either Phase 2 sizes an already-classified issue as Large, **or** the Phase 0.5 **size pre-score** (the outermost gate) predicts likely-Large from the issue body and routes here **before** paying for the parent's governance fork — a parent fork a decomposition would only invalidate. Both land on the same `/sge:build-ready-audit` #872 fold below, which classifies the children once; the size pre-score never re-implements per-child classification.
