---
description: Use when committing work in a WTP/SGD repo — at the end of an implementation slice, before opening a PR, or whenever changes are ready to be recorded with quality gates and SGD traceability trailers. Also use when another skill says "commit via /sgd:commit". Not for amending history or interactive rebase.
...
## Role
Run quality gates, scan for secrets, write the SGD traceability trailers, commit, and push — the single canonical commit entry point for all SGD implementation workflows.
...
Quality-gated, SGD-traceable commit and push.

**This skill is the single canonical owner of two things:**

1. The **SGD trailer convention** (`Spec:` / `SGD-Override:` semantics, below).
2. The **quality-gated commit flow** (gates → secrets scan → commit → guarded push).

Sibling skills (/sgd:sgd-implement, /sgd:pr-fix, /sgd:refactor, /sgd:implement-issue, /sgd:tdd-workflow) do not restate this logic — they say "commit via /sgd:commit" and this file must be sufficient on its own.
...
Example: `/sgd:commit retry backoff fix SPEC-031 --no-push` → message hint "retry backoff fix", spec `SPEC-031`, no push.

## Modes

| Mode | Quality gates | Secrets scan | Commit | Push |
|---|---|---|---|---|
| `/sgd:commit` (default) | yes | yes | yes | yes (guarded) |
| `/sgd:commit --no-push` | yes | yes | yes | **skipped** |
...
**Contract with /sgd:sgd-implement:** intermediate TDD slice commits use `/sgd:commit --no-push`; the final commit of the feature uses plain `/sgd:commit` to gate and push the whole branch.
...
Some repos keep a machine-generated coherence artefact (e.g. `docs/sgd-dag.json` / `docs/coherence.md`) built from the capability model and spec files by a repo-declared script — check for an npm script literally named `build-dag` (in `docs-site/package.json`, the repo root `package.json`, or wherever the repo's own scripts live) or an equivalent convention named in `CLAUDE.md`. If one exists **and** the staged diff touches that generator's own source paths (its header comment or `CLAUDE.md` names them — typically the capability-model file and the spec directory), run it now and stage whatever it changes (e.g. `git add docs/sgd-dag.json docs/coherence.md`) so the regenerated output lands in the **same commit** as the change that triggered it — never a stale doc alongside fresh source. Skip silently if the repo declares no such generator; most repos won't have one.

### 1.6. TDD slice check (test evidence, issue #784)

Refuse to commit an **implementation-only slice** — a staged diff that touches a production-path file with **no staged test-path file** — unless the commit will carry an `SGD-Override: TDD; <reason>` trailer. This is the commit-time layer of #784's TDD gate, between the in-session `tdd-guard.sh` hook (nudges while editing) and the `require-test-evidence.yml` CI gate (the diff-based backstop); catching it here is cheaper than either.

Classify staged files the same way both of those mechanisms do:

- Read `.sgd/test-map.yml` if present (production_paths / test_paths / exempt_paths globs); otherwise use the built-in defaults documented in that file's template (`skills/sgd-init/templates/test-map.yml`) — common source extensions as production, `*.test.*`/`*.spec.*`/`tests/**`/`__tests__/**` as test, docs/config/lockfiles as exempt.
...
- *Add tests now* — stop here, let the user (or you, via `/sgd:tdd-workflow`) stage a failing test first, then re-run `/sgd:commit`;
- *SGD-Override: TDD* — the user confirms this slice intentionally ships without a paired test (legacy code, a spike, infra/config miscategorised as production) and gives a reason (≥10 chars) — proceed, and this becomes the commit's trailer (see Step 5 — the `TDD` override reuses the exact same `SGD-Override: <TOKEN>; <reason>` shape as the spec-trailer convention, so it satisfies both gates with one line);
- *Abort*.
...
### 4. Detect whether the repo is SGD-governed (stack-agnostic)

The repo follows the SGD change protocol if **any** of these hold:
...
- b) The repo's CLAUDE.md (or docs it points to) mentions the SGD trailer convention (`Spec:` / `SGD-Override:` trailers, change protocol).
- c) Spec directories exist — e.g. `docs/features/`, `docs/specs/`, or a `docs/*/specs` layout.

Do **not** key detection on `.husky/` or any other toolchain-specific layout — SGD repos may be TypeScript, Java, C#, Python, anything.

If none of these hold, the repo is not SGD-governed: skip the trailer (step 5) entirely and commit with just the conventional message + co-author line.

### 5. The SGD trailer convention (canonical definition)

Every commit in an SGD-governed repo carries **exactly one** change-protocol trailer so it is traceable through the governance cascade and passes the commit-msg hook:

- `Spec: SPEC-NNN` (or `SGD-NNN`) — the feature spec this commit implements. Use when the work traces to a single spec.
- `SGD-Override: <STEP>; <reason>` — for governance / infra / docs / tooling changes that map to no single spec. `STEP ∈ { LOCATE, READ, IMPACT, PROPOSE, IMPLEMENT, TEST, UPDATE, ALL }` (the protocol step being overridden; `ALL` when no single step applies). The reason must be ≥ 10 characters and say *why* no spec applies — not boilerplate.
...
#### Agent-Id attribution trailer (every commit, SGD repo or not)
...
- The value is read **verbatim from the `SGD_AGENT_ID` environment variable** —
  never invent or reformat it. The agent-registry mints it once at spawn; this
  skill only echoes it.
- It sits in the trailer block alongside `Co-Authored-By:` and the SGD change
  trailer. It is **independent** of the `Spec:` / `SGD-Override:` choice — it is
  attribution, not a change-protocol trailer, and applies in non-SGD repos too.
- **If `SGD_AGENT_ID` is unset or empty** (e.g. a human-driven commit, or a
...
- **SGD-governed repo but no SPEC-NNN known** → never invent one, and never commit trailer-less. Derive the trailer **mechanically** (below).
...
3. **No spec anywhere** → construct `SGD-Override: <STEP>; <reason>` from the work itself:
   ...
   - `<reason>` — ≥ 10 characters, concrete, never boilerplate: say what the change is and *why no spec governs it*, citing the issue when one exists. Example: `SGD-Override: IMPLEMENT; process fix for #1173, no governing spec`.
4. **Genuinely ambiguous** (multiple candidate specs, no single one named by the issue): in an **interactive** session use **AskUserQuestion** — *Provide Spec-Ref* / *SGD-Override* (user gives or approves step + justification) / *Abort*. In a **headless/unattended** session (subagent lane — no user to ask), never stall on a question: fall back to step 3's override, naming the candidate specs in the reason so a human can re-trace it.

**Hard gate — validate before committing.** In an SGD-governed repo, before running `git commit`, check the drafted message contains a line matching `^Spec: *(SPEC|SGD)-[0-9]+` **or** `^SGD-Override: *[A-Z]+; *.{10,}` — the exact patterns the commit-msg hook and the `require-commit-trailer.yml` CI check enforce (note the STEP token is UPPERCASE and the `;` is required). No match → do **not** commit; fix the trailer first. This skill never hands a trailer-less commit to a repo that has the trailer gate.
...
```bash
# Validate SGD_AGENT_ID before interpolating (step 5): only ^agent-[A-Za-z0-9]+$
...
(Swap the `Spec:` line for `SGD-Override: <STEP>; <reason>` when that's the chosen trailer; omit it entirely in non-SGD repos. The `Agent-Id:` line is emitted only when `AGENT_ID` is non-empty after the validation above — never interpolate `SGD_AGENT_ID` into the heredoc directly. The `${AGENT_ID:+…}` expansion drops the whole line, newline included, when it is empty, so the heredoc is no longer single-quoted: keep the message body free of unescaped `$`, backticks, and `\`.)
...
- Invent a `SPEC-NNN` or write a boilerplate `SGD-Override` reason to get past the hook.
