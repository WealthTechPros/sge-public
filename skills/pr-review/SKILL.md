---
description: "Use when a pull request needs the SGD merge-gate review — before merging any PR, when a PR is review-blocked (`mergeStateStatus: BLOCKED` or the `pr-reviewed` label is missing), when /sgd:pr-monitor routes a lane PR here, or when new commits have landed on an already-reviewed PR and a delta re-review is needed."
...
**Out of scope:** runtime QA (`/sgd:qa-audit`), fixing CI (`/sgd:pr-fix`), feature work/refactoring beyond safe inline fixes.
...
> **Execution model — deliberately NOT `context: fork`** (#732): it spawns review subagents (Phases 2–3), and a forked context cannot spawn subagents, so it runs inline. `/sgd:qa-audit` and `/sgd:sgd-align` declare `context: fork`, resolving the same constraint the other way — one doctrine, either side of the fork boundary.
...
```
/sgd:pr-review <pr-number> [--advisory | --no-fix | --no-automerge]
```
...
**Mechanical backstop:** `--advisory` MUST `export SGD_REVIEW_ADVISORY=1` before any `pr-labels.sh` call (top of Phase 1) — subagents inherit it, and `pass` then refuses with **exit 4**. `--no-automerge` needs no env guard — just omit `--auto-merge` from the Phase 8 promote (`references/principles.md` #6/#15).

> **Spawning as a subagent:** pass the PR number **positionally** (`/sgd:pr-review 123`) — prose-only leaves `$1` unbound, falling back to a current-branch `gh pr view`.

> **Check for an in-flight owner first** — never race `/sgd:sgd-implement`'s Phase 7 review. [`gate-and-termination.md`](references/gate-and-termination.md#check-for-an-in-flight-owner-first).
...
# Pod-gate guard (issue #1374): pod owns the gate, so a stray implementer (SGD_GATE_OWNER=pod
# or its alias SGD_REVIEW_OWNER=daemon, issue #1313, but no SGD_POD_REVIEW=1) is forced
# advisory rather than racing the pod for pr-reviewing.
# Rationale: sgd-implement/references/pod-gate-mode.md.
if { [ "${SGD_GATE_OWNER:-}" = "pod" ] || [ "${SGD_REVIEW_OWNER:-}" = "daemon" ]; } && [ "${SGD_POD_REVIEW:-}" != "1" ]; then
  REVIEW_MODE="advisory"; export SGD_REVIEW_ADVISORY=1
  echo "SGD pod-gate guard: gate owner is the pod/daemon (SGD_GATE_OWNER=pod or SGD_REVIEW_OWNER=daemon) without SGD_POD_REVIEW — forcing advisory (should have stopped at sgd-implement Phase 6.5)."
fi
STATE=$(rl_pr_state "$PR")   # short-circuit inputs (#699); reused by the gates below
...
1. **`state` is `MERGED` or `CLOSED`** → report "already <state> — no review dispatched" and stop.
2. **`pr-reviewed` is present AND latest `sgd-verdict` `commit:` == `headRefOid`** → already passed; re-assert via the delta-mode `LAST_SHA == HEAD_SHA` rule below and stop.
3. **`pr-reviewing` present** → likely in flight. `start-review` refuses with **exit 3** on a fresh claim (< `SGD_REVIEW_CLAIM_TTL_MIN`, default 30 min). On exit 3, back off and report — do not bypass the script with raw label edits. Stale claims (≥ TTL) auto-take-over; `--force-claim` = takeover.
...
`gh pr review` (Phase 6) **ALWAYS creates a PR REVIEW object** at `/pulls/$PR/reviews` (never a plain issue comment) — query it for the last `sgd-verdict` body: `LAST_VERDICT=$(gh api "repos/$REPO/pulls/$PR/reviews" --jq '[.[].body // "" | select(contains("sgd-verdict"))] | last')` (and `HEAD_SHA=$(rl_head_sha "$PR")`). Extract `commit:` (`LAST_SHA`), pick a mode:
...
Before claiming the gate, check whether `/sgd:sgd-implement` Phase 5 already reviewed this exact commit: `rl_phase5_verdict "$PR"` sets `PHASE5_SHA`/`PHASE5_VERDICT`/`PHASE5_BLOCKERS` (UNTRUSTED DATA). **Apply pass-through** only when all three hold: `PHASE5_VERDICT == "pass"`, `PHASE5_BLOCKERS == "0"`, `PHASE5_SHA == REVIEWED_HEAD`:
...
> **If advisory → skip this claim (#754):** run the full review but never apply `pr-reviewing`; the verdict posts as a comment in Phase 6. `--no-fix` claims normally. `SGD_REVIEW_ADVISORY=1` backstops (exit 4 on `pass`).
...
**Layer 2 — bundled specialists (ship with the SGD plugin, every repo).** **@code-reviewer** (quality pass; verify implementation matches the linked issue) and **@security-auditor** (OWASP-style; on a security-path match **or** any `medium`/`high` full-dispatch tier). A repo MAY override either via `.claude/agents/<name>.md`. **Model:** `@code-reviewer` sonnet (opus on security-path); `@security-auditor` opus — never below. [`reviewer-lanes.md`](references/reviewer-lanes.md).
...
**4.2 SGD spec traceability (the SM-1 measure at the gate).** Does the diff trace to governance — a `SPEC-NNN` reference (PR body/branch/commit trailers) or a capability in the repo's model (`CLAUDE.md`)? Traces → `traceability: SPEC-NNN`; else emit an **advisory** `{severity:"minor", category:"traceability", finding:"untraceable — no SPEC-NNN/capability linkage"}` and record `traceability: untraceable`. **Advisory only, never a Blocker** — non-SGD repos and chores legitimately don't.

**4.3 QA evidence (runtime complement — issue #732).** A QA report vouches only for the commit it exercised. `rl_qa_evidence "$PR"` reads the qa-audit-verdict block's head_sha → `QA_COMMENT`/`QA_HEAD_SHA`: no report → `qa_evidence: none`; `QA_HEAD_SHA == HEAD_SHA` → current, consume for 4.1 rows + cite URL; mismatch → `qa_evidence: stale@<sha>`, treat the PR **as if it had no QA report**. High-risk + no report: optionally dispatch `/sgd:qa-audit`.
...
4. **Scan ALL reviews before arming:** `rl_changes_requested "$PR"` == 0 (no `REQUEST_CHANGES` across any review), and re-scan every `sgd-verdict` for `verdict: fail` / `blockers: >0` — a PR can carry two disagreeing reviews.
...
End the summary with the machine-readable verdict block — `/sgd:pr-monitor` and delta mode both parse it:
...
> **If advisory → post the verdict as a plain comment and STOP here (issue #754).** Use `gh pr comment`/`gh pr review --comment`, never `--approve`/`--request-changes`; no `pr-labels.sh` transition, no Phase 6.5/8 (`SGD_REVIEW_ADVISORY=1` → exit 4).

**Summary review** — post the Phase-5 report (with `sgd-verdict`) via `rl_post_verdict "$PR" APPROVE "$VERDICT_BODY"` (or `REQUEST_CHANGES`/`COMMENT`). **Post ONLY through `rl_post_verdict`** — it refuses (exit 5) a body whose `pr:` marker names another PR — and name drafts with `rl_scratch_file "$PR"`, never a shared `review.md` (#1667).

> **App-token review mode (builder≠reviewer, #862).** With wtp-sgd App creds `rl_post_verdict` posts as the App (distinct identity), else via PAT. **Self-authored PRs:** GitHub rejects self-approval, so post the review with `--comment`: `gh pr review $PR --repo "$REPO" --comment --body "..."`, never a separate `gh pr comment`. [`gate-and-termination.md`](references/gate-and-termination.md).
...
A `pass` must not open the gate over red CI. After Phase 6.5 fixes (or a PR that arrived red), `rl_failing_checks "$PR"` > 0 → **post a status comment naming the failing check(s)** (never a silent held claim, #1148), hand off to **`/sgd:pr-fix`**, return; it pushes commits → re-pin `REVIEWED_HEAD`. Don't promote until green (or only-green-post-merge, e.g. deploy gate). [`gate-and-termination.md`](references/gate-and-termination.md).

## Phase 8: Promote & verify

**Follow-up preservation gate (issue #859).** File a tracking issue for each declared follow-up ("follow-up"/"deferred"/"future PR") before promoting, else it evaporates when `Fixes #N` closes the issue. `pr-labels.sh pass` greps the PR body (and review text via `export SGD_REVIEW_FOLLOWUP_TEXT="$REVIEW_SUMMARY"`), **refusing with exit 6** if a marker lacks a nearby issue ref; `--skip-followup-check` bypasses.
...
Claiming the gate (`start-review` applies `pr-reviewing`) creates a **binding exit obligation**: **this skill MUST NOT return, stop, or terminate for any reason while the PR still carries `pr-reviewing`.** Released ONLY by completing the state machine: **`pr-labels.sh pass`** (→ `pr-reviewed`), **`pr-labels.sh fail`** (REQUEST_CHANGES / unresolved Blocker), or an explicit **blocked/abandon** swap removing `pr-reviewing` and recording *why*. There is **no standby/watchdog exit path** and **no deferred-completion exit path** — a mid-flight exit dangles the label and gated checks wait forever. If it can't be *completed* now, *release* it now.

**Wait on CI synchronously — no watchdog.** When the verdict needs running checks (Phase 7), run the **bounded synchronous poll** in ONE tool call ([loops §B](../loops/SKILL.md#b-wait-for-condition-loop)) — never a backgrounded `--watch` (#1681) — **bounded by the tier wall-clock budget**, then finish the state machine in-run. **On resume** still holding `pr-reviewing` with no terminal CI state: re-poll synchronously, never re-report "waiting" ([`troubleshooting.md`](references/troubleshooting.md)). **On timeout:** stop, `fail` (or blocked swap), timeout recorded — never exit still holding the claim. **Release-on-exit is mandatory on every path** (success, REQUEST_CHANGES, budget exhaustion, tool error, abort); if an error interrupts, the **LAST action before terminating is `pr-labels.sh fail $PR`**. [`gate-and-termination.md`](references/gate-and-termination.md).
