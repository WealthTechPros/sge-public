...
The actionable Stage 0 **hold-handling convention** for `/sge:pr-review`, relocated here from
...
`REVIEW_MODE`/`SGE_REVIEW_ADVISORY`). The extended rationale, fail-closed incident record, and the
...
5. **Human hold** — a `hold`/`do-not-merge`/`needs-human`/`blocked` label (**authoritative**) or a sign-off-pending marker → **advisory** (`REVIEW_MODE=advisory; export SGE_REVIEW_ADVISORY=1`): Phase 6 `--comment` only, no transition — even a clean APPROVE must not graduate under a hold. `rl_hold_check` **fails closed** (#1347); the `case` graduates only on `ok`. Record `hold_active: true`.
...
case "$HOLD" in
  ...
  *) REVIEW_MODE="advisory"; export SGE_REVIEW_ADVISORY=1   # hold:* + any unrecognised/empty fail closed (#1347)
