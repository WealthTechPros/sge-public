...
#   source "$SGE_ROOT/skills/pr-review/review-lib.sh"
#
...
#
# HTML-comment tracking (issue #2331, bypass 10 -- docs/schemas/
# sge-verdict-block.md). A fence opener hidden inside `<!-- ... -->` is
# invisible in the rendered view (CommonMark), so it must never be treated as
# a real verdict fence and its content must never be extracted/trusted.
# check-review-independence.sh's verdict_scan already tracks comment spans
# this way; this mirrors it exactly (same three-clause state machine,
# evaluated before fence-opener detection) so this function stays in parity
# with both verdict_scan and its own sibling rl__verdict_fence_count (fixed
# under this same issue) per the schema doc's "Implementations" table.
#
# The comment clauses are gated on `!inb` (issue #2371, narrower sibling of
# #2368): CommonMark only recognises `<!-- ... -->` as an HTML-comment span
# OUTSIDE an open code fence -- inside one it is literal fence content, same
# as any other text. Running the comment clauses unconditionally (BEFORE
# checking `inb`) let a `<!--`/`-->` pair that straddled a genuine fence's
# CLOSER swallow that closer as "still in a comment", silently merging the
# fence with whatever real sge-verdict fence followed it -- instead of
# leaving both fences intact so `rl__verdict_fence_count`'s multi-fence
# ambiguity check (already correct; it has no `inb` state) can fail closed.
rl__verdict_block() {
  printf '%s\n' "${1:-}" | awk '
    { sub(/\r$/, "") }
    !inb && in_comment {
...
# HTML-comment tracking (issue #2331, bypass 10 -- docs/schemas/
# sge-verdict-block.md). A fence opener hidden inside `<!-- ... -->` is
# invisible in the rendered view (CommonMark), so it must never count as a
# verdict candidate. check-review-independence.sh's verdict_scan already
# tracks comment spans this way; this mirrors it exactly (same three-clause
# state machine, evaluated before fence-opener detection) so the two parsers
# stay in parity per the schema doc's "Implementations" table.
#
# Leading whitespace is capped at 0-3 chars (not unbounded): CommonMark
# treats 4+ leading spaces as an indented code block, not a fence -- this
# matches check-review-independence.sh's opener regex and the canonical
# schema (docs/schemas/sge-verdict-block.md, bypass 10; issue #2332).
#
# The cap is enforced via a plain-string length check (not a `{0,3}` regex
# interval) on purpose (issue #2391): mawk 1.3.4's regex compiler panics
# ("REcompile() - panic: values still on machine stack") when a bounded
# interval expression is combined with this pattern's alternation + optional
# trailing group in one regex -- reproduced deterministically in CI's
# ubuntu-24.04 mawk job, silently emptying the fence count and opening the
# decoy-fence guard. Splitting the whitespace cap out into ordinary string
# ops (safe under both gawk and mawk) avoids the crash while keeping
# identical CommonMark semantics.
rl__verdict_fence_count() {
  printf '%s\n' "${1:-}" | awk '
    ...
    }
    {
      ...
      if (tolower(body) ~ /^(```+|~~~+)sge-verdict([[:space:]].*)?$/) n++
    }
    END { print n + 0 }
  '
}
...
#
# #2383 extends this same distinction to the "no review id found" branch: if
# no review object exists at all, this also checks the PR's plain issue
# comments for an sge-verdict fence before giving up. Finding one there means
# a verdict WAS posted, just not as a review -- the exact #2209 incident
# shape (App comments kept succeeding while review-posting silently broke).
# That gets its own diagnostic ("posted as a COMMENT, not a review"),
# distinct from "nothing was posted at all".
#
...
rl_verify_verdict_posted() {
  ...
  if [ -z "$review" ]; then
    # Reviews-vs-comments health check (#2383, extends #2210/#2292): the
    # #2209 incident's specific failure mode was the App posting plain issue
    # comments successfully while its review-posting silently stopped for a
    # day -- from the caller's side that can look identical to "posted
    # nothing" unless this function actually looks. Before returning the
    # generic failure, check whether an sge-verdict fence landed as a plain
    # PR comment instead of a review object -- if so, say THAT, not "did not
    # land" (a fenced comment did land; it just isn't the review artefact
    # rl_post_verdict was supposed to create).
    local comments comment_fence
    comments=$(gh api --paginate --slurp "repos/${repo}/issues/${pr}/comments" 2>/dev/null | \
      jq -c 'if type=="array" then (add // []) else empty end' 2>/dev/null)
    if [ -n "$comments" ]; then
      # base64-per-line (not raw `.body`): a comment body is multi-line, and
      # piping raw bodies through `while read` would split each body across
      # several reads -- rl__verdict_block would then see fence-opener and
      # fence-closer lines in isolation and never match. One base64 blob per
      # comment keeps each body atomic for the read loop.
      comment_fence=$(printf '%s' "$comments" | jq -r '.[].body // empty | @base64' 2>/dev/null | while IFS= read -r cb64; do
        cbody=$(printf '%s' "$cb64" | base64 -d 2>/dev/null)
        [ -n "$(rl__verdict_block "$cbody")" ] && { echo found; break; }
      done)
      if [ "$comment_fence" = "found" ]; then
        echo "rl_verify_verdict_posted: no review OBJECT with id ${id} exists on PR #${pr}, but a plain issue comment carrying an sge-verdict fence was found -- a verdict was posted as a COMMENT, not a review (issue #2209: reviews vs. comments). This is not equivalent to a posted review and must not be treated as one." >&2
        return 1
      fi
    fi
    echo "rl_verify_verdict_posted: no review found with id ${id} on PR #${pr}, and no sge-verdict fence found in plain comments either -- nothing was posted at all (issue #2292)" >&2
    return 1
  fi
  ...
  # Defense in depth (issue #2371): rl__verdict_block only ever returns the
  # FIRST fence's content -- a non-empty result alone does not prove the body
  # is unambiguous. rl_verdict_findings_unverified already fails closed on a
  # multi-fence body before POSTING a verdict; this mirrors that same posture
  # on the READ side, so a body carrying more than one sge-verdict fence
  # (decoy, or two genuine fences a parser edge case merged) is never treated
  # as a confirmed posted verdict either.
  local fcount
  fcount=$(rl__verdict_fence_count "$body")
  if [ "${fcount:-0}" -ne 1 ]; then
    echo "rl_verify_verdict_posted: review ${id} on PR #${pr} has ${fcount:-0} sge-verdict fences in its body -- ambiguous, refusing to confirm a single verdict (issue #2371)" >&2
    return 1
  fi
