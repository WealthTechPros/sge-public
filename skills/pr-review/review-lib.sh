...
# Ensure the PR body LINKS issue <n> — appends "Fixes #n" unless a link for #n
# is already there.
#
# `Part of #n` counts as a link and is left alone (#2241). It is the deliberate
# non-closing form written by /sge:sge-implement Phase 3, and kept at Phase 6
# when the PR does not satisfy every acceptance criterion or the issue is a
# `tracking`/`epic` umbrella — see sge-implement/references/close-keyword.md.
# Treating it as "unlinked" and appending `Fixes #n` re-adds the closing keyword
# one phase after Phase 6 deliberately withheld it, which auto-closes a partial
# issue on merge: the exact defect #2241 exists to prevent, restored here.
rl_ensure_closing_link() {
  local pr="$1" n="$2" repo body
  repo=$(rl__repo) || return 1
  body=$(gh api "repos/$repo/pulls/$pr" --jq .body) || return 1
  # No cross-repo prefix on the alternation: a body reference to an ISSUE IN
  # ANOTHER REPO (e.g. `Part of otherorg/otherrepo#$n`) is not a link to this
  # repo's issue #$n and must not satisfy this same-repo linkage check.
  if ! printf '%s' "$body" | grep -qiE "(clos(e|es|ed)|fix(es|ed)?|resolv(e|es|ed)|(^|[^[:alnum:]])part[[:space:]]+of)[[:space:]]+#$n([^0-9]|$)"; then
