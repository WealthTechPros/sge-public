---
description: Use when preparing to implement an SGD feature spec — before any branch, plan, or code exists — when asked to preflight or check build-readiness of a SPEC-NNN, or when /sgd:sgd-implement Phase 1 delegates its entry-criteria checks.
...
## Role
Gate an SGD feature spec as ready-to-build — check completeness, spec status, dependencies, and test infrastructure — then post a structured report on the issue, before any branch or code is created.

## Out of scope
- Implementing the spec (hands off to `/sgd:sgd-implement`)
- Deep issue triage (use `/sgd:deep-dive` for unclear specs)
- Non-spec (plain issue) readiness checks (use `/sgd:build-ready-audit`)
...
Read-only pre-implementation checklist for an SGD feature spec. It establishes
...
2. **Dispatched** — `/sgd:sgd-implement` Phase 1 invokes it headlessly. In
...
```
/sgd:sgd-preflight <SPEC-NNN or issue#>
```
...
> spec lives in. When `/sgd:sgd-implement` Phase 1 dispatches this from a
...
Before reading any file or calling `gh`, call `search_nodes` for the spec id and issue number (if sgd-memory is available; skip silently if not).
...
- **Dependencies** — what other SGD features must exist first?
...
L0–L8 artefact stack (epic sgd#785; format contract: `docs/sgd-digest-schema.md`
in the sgd repo). How deep to read past the digest is set by the change's
**tier** (issue sgd#809) — deeper context loads only for higher-risk work:

1. **Digest first (always).** Read `docs/sgd-digest.md` (location per CLAUDE.md
   ...
   the Step 5 report (`scripts/build-sgd-digest.mjs` in the sgd repo
   generates one).
...
- **Cascade-citation gate** — the spec cites its place in the governance
  ...
  untraceable at the SM-1 check in `/sgd:sgd-review`).
...
Score the spec with the **canonical SGD complexity rubric owned by
`/sgd:sgd-implement` (Phase 2)** — count the spec's data models, service
...
table here; if you need the exact weights, read them from the sgd-implement
skill. Record the numeric score and its band (small / medium / large-split)
in the report — `/sgd:sgd-implement` consumes this score and does not
recompute it.
...
- **`standard` → `scoped`**: resolve which *other* specs/ADRs govern the touched
  ...
  node "${CLAUDE_PLUGIN_ROOT:-.}/scripts/resolve-context-scope.mjs" \
    --dag docs/sgd-dag.json --paths "<comma-separated Step 2C planned paths>"
  ```
...
one question, lettered choices, mirroring sgd-implement's entry gate:
...
`openQuestions[]` listing gating QDs and gaps) — `/sgd:sgd-implement` Phase 1
...
### Complexity Score
[N] — [small / medium / large-split] (canonical rubric: /sgd:sgd-implement Phase 2)
