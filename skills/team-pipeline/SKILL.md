...
> **`--dry-run` mode:** the gate still runs in full. A `--dry-run` that would fail
> the gate must print the block and exit — no report for an unsafe config.
> **Forgejo?** [host-routing](references/host-routing.md)
