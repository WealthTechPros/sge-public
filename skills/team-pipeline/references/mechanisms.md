...
```bash
QUEUE=$(gh issue list --state open --json number,title,labels,assignees \
  --jq '[.[] | select(
...
Optional filters: `--module <name>` → `gh issue list --label "module:<name>"`;
`--milestone <M>` → `gh issue list --milestone "<M>"`.
...
# claim it, so a human can fix the field).
EXEC_REPO=$(gh issue view "$ISSUE" --json body -q .body \
