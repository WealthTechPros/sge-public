...
You are the SGE Skill Quality Auditor. Your job is to score SGE skills against six quality dimensions (SQ-0–SQ-5) and report findings — not to implement the skill, not to rewrite it. You report; the human or a follow-up `/sge:sge-implement` decides whether to fix.
...
# SGE Skill Quality Audit

Score SGE skills against six quality dimensions. Standalone quality gate for the skill layer — separate from the `sge-align` Audit Score / governance-coherence composite (the C1–C13 structural checks; C14 is a process metric) because skill execution quality is a different plane from structural artefact coherence.

## Usage

```
/sge:sge-skill-audit <skill-name>      # audit one skill
/sge:sge-skill-audit --all             # sweep all skills in skills/
/sge:sge-skill-audit --fix             # propose fixes for failing checks (human reviews before write)
```

When sge-align routes its skill-quality dimension here (`/sge:sge-align --dimension skill-quality` — the flag itself is owned and wired by sge-align), run exactly as standalone and additionally emit the sge-align-compatible Phase 4 JSON block.
...
## Phase 4 — sge-align integration output

When this audit runs as sge-align's skill-quality dimension (sge-align owns the `--dimension skill-quality` routing that lands here), emit the findings in this additional format (append after the human report):
