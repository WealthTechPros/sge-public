---
description: Use when onboarding a new product or a greenfield repo onto SGD — when a repo has no Vision, capability model, or feature specs yet; when the user asks to set up SGD, seed governance artefacts, or run product intake; or when an intake/brief document needs turning into SGD seed artefacts. Not for auditing an already-seeded repo — that is /sgd:sgd-align.
...
## Role
Interview for a new product, then seed the SGD governance layers — Vision, capability model, feature specs — so the repo is ready for `/sgd:sgd-implement` and `/sgd:sgd-align`.

## Out of scope
- Auditing an already-seeded repo (that is `/sgd:sgd-align`)
- Implementing any feature (hands off to `/sgd:sgd-implement`)
...
Take a new product idea (or a greenfield repo) from a blank page to the inputs SGD needs —
...
```
/sgd:sgd-init [intake-doc path]
```

Run it in the target repo. For a brand-new repo, run the greenfield bootstrap first
(`npx sgd-init --phase=c`) so the SGD scaffolding exists, then run this skill to fill it in.
...
- Existing SGD artefacts: !`ls -d docs/vision.md docs/features docs/decisions docs/sgd .claude/product-context/capability-model.yaml 2>/dev/null || echo "(none found at canonical paths)"`
...
These are the canonical locations this skill seeds — the **same map `/sgd:sgd-align`
...
`/sgd:sgd-align` discovers artefact locations by reading `CLAUDE.md`, not by guessing.
...
| QD registry (stakeholder questions) | `docs/sgd/questions.md` |
| Change protocol | `docs/sgd/change-protocol.md` |
...
`/sgd:spec-validate <spec> <fixture>` against a demo fixture to confirm each invariant
holds.
...
it looks covered and isn't (this is exactly the gap SGD#762 exists to close: a test
...
citation keys `/sgd:sgd-align` check C7 reads, so seeded repos pass the sweep from day one:
...
- **Location:** `docs/sgd/questions.md` — one registry file per repo (or the repo's
...
- **Closure:** a QD is closed by recording the decision in the registry **and** updating
  ...
  spec body). `/sgd:sgd-align` check C8 flags QDs open past threshold.

Do **not** create any external database without explicit approval.

## Step 7 — Scaffold the change-protocol guardrails

Propose adding the SGD commit-trailer guardrails so every future commit traces to a spec
(AI proposes, human disposes — write only after approval):

- `docs/sgd/change-protocol.md` — from `${CLAUDE_PLUGIN_ROOT}/skills/sgd-init/templates/change-protocol.md`
  (the 7-step protocol; tailor the wording to the repo).
- A `commit-msg` hook — from `${CLAUDE_PLUGIN_ROOT}/skills/sgd-init/templates/commit-msg`
  (warns when a commit lacks a `Spec:` / `SGD-Override:` trailer; accepts `SPEC-NNN` or
...
```sh
mkdir -p .githooks
cp "${CLAUDE_PLUGIN_ROOT}/skills/sgd-init/templates/commit-msg" .githooks/commit-msg
...
The **trailer convention itself** (`Spec:` / `SGD-Override:` semantics, one-trailer rule,
never `--no-verify`) is canonically documented in `/sgd:commit` — this skill carries only
the **hook definition** it installs. With the hook in place, `/sgd:commit` emits the
matching trailer automatically (and `/sgd:sgd-implement` commits through it), and the
warning becomes blocking once the repo opts into Phase 2.

## Step 7b — Seed the Agent Security (Zero-Trust) dimension baseline

After the change-protocol guardrails are in place, seed the **C11 Agent Security baseline** so the first `/sgd:sgd-align` run has a starting posture rather than reporting every control as 🔴 with no context.

Propose the following (AI proposes, human disposes — write only after approval):

1. **Initial posture check** — run `/sgd:sgd-align --dimension agent-security --dry-run` (if the repo is ready) or run the C11 script (`skills/sgd-align/assets/check-agent-security.sh` in the plugin) against the current repo state and report the starting score (e.g. `2/5 controls passing at onboarding`).

2. **Governance-posture seed record** — propose adding `docs/sgd/agent-security-posture.md` with the initial C11 result, the date, and the audited SHA. This gives the first "before" snapshot so future sweeps can report a delta (`was 2/5 at onboarding, now 4/5`).

   Template (fill in with actual check results — five controls; ZT-6 was dropped as a self-referential vanity control, sgd#842):

   ```markdown
   # Agent Security Posture — <repo>

   Seeded by /sgd:sgd-init at onboarding. Re-assess with `/sgd:sgd-align --dimension agent-security`.
...
- `.sgd/test-map.yml` — from `${CLAUDE_PLUGIN_ROOT}/skills/sgd-init/templates/test-map.yml`. Tailor the commented-out `production_paths`/`test_paths` globs to the repo's actual languages and layout before uncommenting them; leave `mode: advisory` — never seed a new repo straight into blocking.
...
The gate reads `.sgd/test-map.yml` for its production/test path classification, or falls back to built-in language-aware defaults if the file is absent or left with no uncommented lists — so it produces *some* signal even before this file is tailored. The in-session companion (`hooks/tdd-guard.sh`, ships with the plugin, no per-repo install needed) reads the same file for its warn-by-default nudge.

Skip this step and note it in the Review Package if the repo has no CI, or is a docs-only/ideation-stage repo with no runtime code to gate (see `org-context.md`'s "SGD ideation only" repos).
...
- `.sgd/regulated-paths.yml` — from `${CLAUDE_PLUGIN_ROOT}/skills/sgd-init/templates/regulated-paths.yml`. Tailor the commented-out `regulated_paths` globs to the files that render regulated numbers, and set the `signers:` list to the GitHub handles authorised to sign off. Leave `mode: advisory` — never seed a new repo straight into blocking.
...
Propose adding (or merging into) the repo's `CLAUDE.md` a short **SGD artefact map**
...
`/sgd:sgd-align` resolves every layer's location from `CLAUDE.md`, so the seeded repo is
...
> **Default governance read:** load [`docs/sgd-digest.md`](docs/sgd-digest.md) — the
...
> read deliberately). Regenerate with `node scripts/build-sgd-digest.mjs`; CI verifies
> freshness with `node scripts/build-sgd-digest.mjs --check`.

The digest is produced by `scripts/build-sgd-digest.mjs` (seeded by the enabler in #805);
...
`/sgd:sgd-preflight` and `/sgd:sgd-implement` to build the first spec. See also
`/sgd:sgd-preflight` and `/sgd:tdd-workflow`.
